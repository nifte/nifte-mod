			...
			if (this.isNoAi()) {
				...
				if (this.level() instanceof ServerLevel level) {
					...
					if (targetLocation != null) {
						...
						Vec3 dir = new Vec3(Mth.sin(this.getYRot() * (float) (Math.PI / 180.0)), this.getDeltaMovement().y, -Mth.cos(this.getYRot() * (float) (Math.PI / 180.0)))
							.normalize();
				...
				if (this.level() instanceof ServerLevel serverLevel && this.hurtTime == 0) {
					this.knockBack(
						serverLevel,
						serverLevel.getEntities(this, this.wing1.getBoundingBox().inflate(4.0, 2.0, 4.0).move(0.0, -2.0, 0.0), EntitySelector.NO_CREATIVE_OR_SPECTATOR)
					);
					this.knockBack(
						serverLevel,
						serverLevel.getEntities(this, this.wing2.getBoundingBox().inflate(4.0, 2.0, 4.0).move(0.0, -2.0, 0.0), EntitySelector.NO_CREATIVE_OR_SPECTATOR)
					);
