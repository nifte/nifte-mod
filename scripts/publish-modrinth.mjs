...
if (!token) {
  console.error(
    'Set MODRINTH_TOKEN first. Create a token at https://modrinth.com/settings/pats',
  )
  console.error(
    'Required scopes: Create projects, Write projects, Create versions',
  )
  process.exit(1)
}
...
async function createProject() {
  const data = {
    ...
    title: 'Nifte Mod',
    description:
      'A client-side quality-of-life mod with toggleable HUD, inventory, camera, and crafting features.',
    categories: ['utility'],
...
const existing = await fetch(`${api}/project/${slug}`, { headers }).then(
  async (response) => {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to look up project (${response.status}): ${await response.text()}`,
      )
    }
...
const alreadyUploaded =
  ...
  versions.some((version) => version.version_number === '1.0.0')
if (alreadyUploaded) {
