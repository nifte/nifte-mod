...
public class LevelRenderer implements AutoCloseable {
				...
				pass.executes(
					() -> {
						...
						if (state.skybox == Skybox.END) {
							...
							this.skyRenderer
								.renderSunMoonAndStars(poseStack, state.sunAngle, state.moonAngle, state.starAngle, state.moonPhase, state.rainBrightness, state.starBrightness);
	...
	public TemporaryCollection collectPerFrameRenderThreadGizmos() {
	...
	private void finalizeGizmoCollection() {
		...
		for (GizmoInstance instance : this.renderThreadGizmos.drainGizmos()) {
	...
	public void addMainThreadGizmos(final List<GizmoInstance> mainThreadGizmos) {
